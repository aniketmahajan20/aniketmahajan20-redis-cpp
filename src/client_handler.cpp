...
void ClientHandler::client_handler(int client_fd){
    ...
    while (true){
        ...
        if (bytesReceived == -1){
            std::cerr << "Error in recv(): " << strerror(errno) << "\n";
            break;
        }
        if (bytesReceived == 0){
        ...
        // std::cout << "Received Bytes: " << recv_str << "\n";
...
void ClientHandler::response_handler(int client_fd, std::string recv_str){
    ...
    while(true){
        ...
        if (!response.empty()){
            ...
            // std::cout<< "Notified but the response was empty: \n";
...
void ClientHandler::worker_thread() {
    while (true) {
        ...
        if (!task_queue.empty()) {
            ...
            task_queue.pop();
            // std::cout << std::this_thread::get_id() << ": A Task found. Now executing it\n";
